...
function getProductForCart(product) {
    ...
    addProduct.addEventListener("click", function () {
        ...
        if (quantityProduct.value !== 0 && choiceColor.value !== "") {
            ...
            if (cartSaved) {
                // Permet de controler l'existence d'un produit dans le panier et de comparer si il y a déjà un produit existant (même ID et même couleur)
                ...
                if (productControl) {
                    ...
                    //MAJ de la productControl.quantity pour avoir le bon nombre dans le panier
                    productControl.quantity = finalQuantity;

                    saveCart(cartSaved);
                    console.log(
                        "La quantité sélectionné est de " + myProduct.quantity
                    );
                    console.log(
                        "La nouvele quantité est de est de " + finalQuantity
                    );
                    //Si il n'y a pas de produit identique alors on pousse le nouveau produit
                } else {
